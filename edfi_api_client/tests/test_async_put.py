...
async def test_async_put(output_secret: str, input_secret: str, verbose: bool = False):
    """
        ...
        for namespace, rr in [
            ...
            *resources
            ...
            try:
                ...
                error_log = input_endpoint.put_id_rows(((row['id'], row) for row in rows_iterator))
                print(error_log.count_messages())
                end_time = time.time()
                ...
                error_log = await input_endpoint.async_put_id_rows(((row['id'], row) async for row in rows_iterator))
                print(error_log.count_messages())
                end_time = time.time()
                print(f"\n    Runtime: {end_time - start_time}")
...
if __name__ == '__main__':
    ...
    asyncio.run(test_async_put(OUTPUT_SECRET, INPUT_SECRET, verbose=VERBOSE))
